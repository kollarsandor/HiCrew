...
def get_model(args):
    ...
    if 'gpt' in model_name:
        # 使用 OpenAI API，传入 api_key 和 base_url
        model = GPT(args.api_key, model_name, temperature, base_url="https://opus.gptuu.com" )
        return model
...
class GPT(Model):
    def __init__(self, api_key, model_name, temperature, base_url=None):
        ...
        # 初始化 OpenAI 客户端，添加 api_key 和可选的 base_url
        client_kwargs = {"api_key": api_key}
        if base_url:
            client_kwargs["base_url"] = base_url + '/v1'  # 添加 '/v1' 路径
...
# # 初始化模型
# model = GPT(api_key, model_name, temperature, base_url=base_url)

# # 设置提示词
# system_prompt = "你是一个友好的AI助手。"
# user_message = "你好，请给我讲一个笑话。"

# # 调用模型获取回答
# response, info = model.forward(system_prompt, [user_message])
