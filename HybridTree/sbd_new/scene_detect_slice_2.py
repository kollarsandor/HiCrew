...
##--------------------------文件加载---------------------------------
# 指定视频文件夹路径
video_folder_path = "./Egoschema_videos/videos/videos"
...
# 处理符合条件的视频
output_data = {}
total_videos = len(video_uids_in_folder)
processed_videos = 0
...
for video_uid in video_uids_in_folder:
    ...
    speed = processed_videos / elapsed_time if elapsed_time > 0 else 0
    
    print(f"\r进度: {processed_videos/total_videos*100:.1f}% | {processed_videos}/{total_videos} | {elapsed_time:.1f}s/{elapsed_time/processed_videos*total_videos:.1f}s | {speed:.2f}视频/秒", end="")
    
    video_path = os.path.join(video_folder_path, f"{video_uid}.mp4")
    if not os.path.exists(video_path):
        print(f"\n跳过 {video_uid}: 视频文件不存在。")
        continue
    #------基础分割------#
